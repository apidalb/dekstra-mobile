...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  void _next() {
    ...
    if (_currentStep == 0) valid = _formKey1.currentState!.validate();
    else if (_currentStep == 1) valid = _formKey2.currentState!.validate();
    else if (_currentStep == 2) valid = _formKey3.currentState!.validate();
      ...
      if (_otpMethod == null) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Pilih metode pengiriman OTP',
              style: GoogleFonts.poppins()),
          backgroundColor: Colors.red,
        ));
        return;
      ...
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(
            builder: (_) => HomeScreen(userName: _namaController.text)),
        (route) => false,
      );
      return;
    }
  ...
  // ── Step 5: OTP ────────────────────────────────────────────────────────────
  Widget _buildStep5() {
    final maskedPhone = _maskPhone(_teleponController.text);
    final maskedEmail = _maskEmail(_emailController.text);
    return Column(
      children: [
        const SizedBox(height: 8),
        _otpOption('phone', Icons.phone_outlined,
            'Kirim Melalui Nomor Telepon',
            'Kirim kode OTP melalui nomor $maskedPhone'),
        const SizedBox(height: 12),
        _otpOption('email', Icons.email_outlined,
            'Kirim Melalui Email',
        ...
        children: [
          ...
          Expanded(
            child: ElevatedButton(
                onPressed: _next, child: Text(nextLabel)),
          ),
        ],
      ),
    );
  ...
  String _maskPhone(String v) {
    ...
    return '${v.substring(0, 2)}${'*' * (v.length - 4)}${v.substring(v.length - 3)}';
  }
