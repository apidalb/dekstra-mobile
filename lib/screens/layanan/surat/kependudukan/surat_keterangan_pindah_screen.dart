...
class _AnggotaPindah {
  ...
  String? jk;
  final shdkCtrl = TextEditingController();
  void dispose() { nikCtrl.dispose(); namaCtrl.dispose(); shdkCtrl.dispose(); }
}
...
class _PindahState extends State<SuratKeteranganPindahScreen> {
  ...
  static const _shdkList = ['Kepala Keluarga','Suami','Istri','Anak','Menantu',
    'Cucu','Orang Tua','Mertua','Famili Lain'];

  @override
  void dispose() {
    for (final c in [_namaCtrl, _nikCtrl, _noKkCtrl, _tempatCtrl, _tglCtrl,
      _pekerjaanCtrl, _kwrgCtrl, _alamatCtrl, _alasanCtrl, _alamatTujuanCtrl]) { c.dispose(); }
  ...
  void _submit() async {
    ...
    if (!mounted) return;
    showSuccessDialog(context, 'Surat Keterangan Pindah Penduduk');
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Surat Ket. Pindah'),
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            namaSuratCard('SURAT KETERANGAN PINDAH PENDUDUK'),
            ...
            sectionCard(children: [
              ...
              const SizedBox(height: 6),
              TextFormField(controller: _alasanCtrl, maxLines: 2,
                style: GoogleFonts.poppins(fontSize: 13),
                decoration: const InputDecoration(hintText: 'Masukkan alasan pindah'),
                validator: (v) => validateRequired(v, 'Alasan')),
            ]),
            const SizedBox(height: 16),
                ...
                child: sectionCard(children: [
                  ...
                  DropdownButtonFormField<String>(initialValue: a.shdkCtrl.text.isEmpty ? null : a.shdkCtrl.text,
                    ...
                    items: _shdkList.map((e) => DropdownMenuItem(value: e,
                        child: Text(e, style: GoogleFonts.poppins(fontSize: 13)))).toList(),
                    onChanged: (v) => setState(() => a.shdkCtrl.text = v ?? ''),
